...
const PAGES = [
  {
    ...
    sections: [
      ...
      ['2. Data We Collect', [
        'When you create an account or complete a purchase we collect your full name, email address, optional contact number, and payment method metadata (such as the gateway used and transaction reference). We never store full credit or debit card numbers — all card processing is handled by PCI-DSS compliant gateways including Visa, Mastercard, EasyPaisa, JazzCash, Binance Pay, and Apple Pay. If you sign up using a social provider (Google or Facebook) we receive your name, email, and provider account identifier from that platform, nothing more.',
